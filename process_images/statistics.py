...
class StatsAccumulator:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "by_category": {
                cat: {s.value: c for s, c in counts.items()}
                for cat, counts in sorted(cat_counts.items())
            },
            "by_flag": {
                f.value: c for f, c in sorted(flag_counts.items(), key=lambda x: x[0].value)
            },
            "quality": {
            ...
            "performance": {
                ...
                "avg_fallback_time_s": round(_safe_avg(fallback_times), 4),
    ...
    def to_console(self) -> str:
        ...
        for cat, counts in sorted(d["by_category"].items()):
        ...
        lines.extend(
            [
                ...
                f"  min={q['min_fill_ratio']:.3f}"
                f"  max={q['max_fill_ratio']:.3f}",
            ]
        )
        ...
        lines.extend(
            [
                ...
                f"  Avg per image: {p['avg_per_image_s']:.3f}s",
